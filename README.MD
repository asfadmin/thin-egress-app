...
`pip-compile` to generate the `requirements/requirements.txt` file.

Note that `pip-tools` itself is declared as a development dependency, so the
following commands are sufficient for adding a dependency:
