...
[![codecov](https://codecov.io/gh/asfadmin/thin-egress-app/branch/main/graph/badge.svg?token=Jd5l4IVpkM)](https://codecov.io/gh/asfadmin/thin-egress-app)
...
- `python3.12` for running tests and creating the CloudFormation template
...
```bash
# Touch all the files, so lock thinks there's a change:
touch requirements/*.in
# Build the tea-dependency-builder container for lambda:
make tea-dependency-builder
...
The [End-to-End Tests](./.github/workflows/test-e2e.yml) is an GitHub Action you can deploy with a workflow_dispatch against your stack. It'll deploy your code to `test`, and run the e2e suite against it. More info in the [workflows README](./.github/workflows/README.md).
