...
JWT_MANAGER = JwtManager(
    ...
    session_ttl_in_hours=float(os.getenv("SESSION_TTL_HRS", 24)),
)
...
def get_black_list():
    ...
    if endpoint:
        response = urllib.request.urlopen(endpoint).read().decode()
...
def restore_bucket_vars():
    ...
    if b_map is None:
        log.info(
            "downloading various bucket configs from %s: bucketmapfile: %s",
            conf_bucket,
            bucket_map_file,
        )
...
def try_download_from_bucket(bucket, filename, user_profile, headers: dict, api_request_uuid):
    ...
    try:
        ...
        log.error(
            "ClientError while %s tried downloading %s/%s: %s",
            user_id,
            bucket,
            filename,
            e,
    ...
    try:
        ...
        log.info("%s", {"download": download_stat})
...
def dynamic_url_head():
    ...
    log.debug("attempting to HEAD: %s", param)
    restore_bucket_vars()
...
def dynamic_url():
    ...
    if not entry.object_key:
        log.warning(
            "Request was made to directory listing instead of object: %s",
            entry.bucket_path,
        )
