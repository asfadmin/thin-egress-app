...
def lambda_handler(event, context):
    try:
        ...
        new_policy["Statement"][0]["Condition"] = {
            "IpAddress": {"aws:SourceIp": cidr_list},
        }
        ...
        error_string = (
            ...
            f"for Role {os.getenv('iam_role_name')} "
            f"in region {current_region}: {e}"
        )
        print(error_string)
...
def get_region_cidrs(current_region):
    ...
    output = urllib.request.urlopen("https://ip-ranges.amazonaws.com/ip-ranges.json").read().decode()  # nosec
    ...
    # It's important to filter down the CIDR range as much as possible.
    # Too large can cause the role creation to fail.
