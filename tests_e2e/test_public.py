...
def test_public_images(api_url):
    url = f'{api_url}/{BROWSE_FILE}'
    r = requests.get(url)
...
def test_check_public_obj_prefix(api_url):
    url = f'{api_url}/{OBJ_PREFIX_FILE}'
    r = requests.get(url)

    assert r.status_code == 200


def test_404_on_bad_request(api_url):
    url = f"{api_url}/bad/url.ext"
    r = requests.get(url)
